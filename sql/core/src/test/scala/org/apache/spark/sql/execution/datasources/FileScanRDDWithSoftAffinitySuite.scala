...
class FileScanRDDWithSoftAffinitySuite extends QueryTest
  with SharedSQLContext with PredicateHelper {
  ...
  test("Soft Affinity Scheduler for FileScanRDD") {
    ...
    SparkEnv.get.conf.set(SoftAffinityConstants.PARAMS_KEY_ENABLED_SOFT_AFFINITY, "true")
    ...
    SparkEnv.get.conf.remove(SoftAffinityConstants.PARAMS_KEY_ENABLED_SOFT_AFFINITY)
    SparkEnv.get.conf.remove(SoftAffinityConstants.PARAMS_KEY_SOFT_AFFINITY_REPLICATION_NUM)
    SparkEnv.get.conf.remove(SoftAffinityConstants.PARAMS_KEY_SOFT_AFFINITY_MIN_TARGET_HOSTS)
  }
}
